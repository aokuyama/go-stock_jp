...
func TestPickNewPosition(t *testing.T) {
	...
	t1, _ := entity.NewPositionTrade("spot_buy", "1234", 100)
	ps.addPositionTrade(t1)
	assert.Equal(t, 1, len(*ps), "ポジションが追加される")
	f := (*ps)[0]
	t2, _ := entity.NewPositionTrade("margin_buy", "1234", 100)
	...
	t3, _ := entity.NewPositionTrade("spot_buy", "1234", 300)
...
func TestAddPayTrade(t *testing.T) {
	var err error
	ps := NewPositions()
	po, _ := entity.NewPositionTrade("margin_buy", "1234", 200)
	err = ps.addPositionTrade(po)
	assert.NoError(t, err)
	err = ps.addPositionTrade(po)
	assert.NoError(t, err)
	assert.Equal(t, 400, (*ps)[0].Quantity.Int())
	py, _ := entity.NewPayTrade("pay_sell", "1234", 200)
...
func TestErrorPayTrade(t *testing.T) {
	...
	py, _ := entity.NewPayTrade("spot_sell", "1234", 100)
	err = ps.addPayTrade(py)
	assert.Error(t, err)
	assert.Equal(t, 1, len(*ps.errors()), "対象がないためエラーが追加される")
	ep := (*ps.errors())[0]
	assert.Equal(t, -100, ep.Quantity.Int())
	po, _ := entity.NewPositionTrade("margin_buy", "1234", 100)
	ps.addPositionTrade(po)
	py2, _ := entity.NewPayTrade("pay_sell", "2234", 100)
...
func TestOverPayTrade(t *testing.T) {
	var err error
	ps := NewPositions()
	po, _ := entity.NewPositionTrade("margin_buy", "1234", 300)
	ps.addPositionTrade(po)
	py, _ := entity.NewPayTrade("pay_sell", "1234", 200)
	err = ps.addPayTrade(py)
	assert.NoError(t, err)
...
func TestCompress(t *testing.T) {
	t.Run("case 1", func(t *testing.T) {
		ps := NewPositions()
		p1, _ := entity.NewPosition("spot_buy", "1234", 500)
		p2, _ := entity.NewPosition("margin_buy", "1234", 400)
		p3, _ := entity.NewPosition("margin_sell", "1234", 200)
		p4, _ := entity.NewPosition("margin_sell", "1234", 100)
		p5, _ := entity.NewPosition("margin_buy", "1235", 100)
...
func TestDiff(t *testing.T) {
	...
	t.Run("case 2", func(t *testing.T) {
		...
		p1, _ := entity.NewPosition("spot_buy", "1234", 500)
		p2, _ := entity.NewPosition("margin_buy", "1234", 400)
		p3, _ := entity.NewPosition("margin_sell", "1234", 300)
		p4, _ := entity.NewPosition("margin_buy", "1235", 200)
