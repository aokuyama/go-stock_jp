...
func TestDiff(t *testing.T) {
	t.Run("no diff", func(t *testing.T) {
		...
		p1, _ := position.New("spot_buy", "1234", 500)
		p2, _ := position.New("margin_buy", "1234", 400)
		p3, _ := position.New("margin_sell", "1234", 200)
		p4, _ := position.New("margin_sell", "1234", 100)
		p5, _ := position.New("margin_buy", "1235", 100)
	...
	t.Run("case 1", func(t *testing.T) {
		...
		assert.Equal(t, 300, (*diff1)[0].Quantity())
		diff2 := DiffPositions(ps2, ps1)
		assert.Equal(t, 1, len(*diff2))
		assert.Equal(t, 200, (*diff2)[0].Quantity())
	})
	t.Run("case 2", func(t *testing.T) {
		ps1 := position.NewPositions()
		p1, _ := position.New("spot_buy", "1234", 500)
		p2, _ := position.New("margin_buy", "1234", 400)
		p3, _ := position.New("margin_sell", "1234", 300)
		p4, _ := position.New("margin_buy", "1235", 200)
		ps1.AddPosition(p1)
		ps1.AddPosition(p2)
		ps1.AddPosition(p3)
		ps2 := position.NewPositions()
		...
		diff1 := DiffPositions(ps1, ps2)
		assert.Equal(t, 1, len(*diff1))
		assert.Equal(t, 500, (*diff1)[0].Quantity())
		diff2 := DiffPositions(ps2, ps1)
		assert.Equal(t, 1, len(*diff2))
		assert.Equal(t, 200, (*diff2)[0].Quantity())
